...
prepare_cmake() {
  if ! which cmake &>/dev/null; then
    ...
    if [ "${USE_CHINA_MIRROR}" = "1" ]; then
        cmake_binary_url="https://ghproxy.net/${cmake_binary_url}"
        cmake_sha256_url="https://ghproxy.net/${cmake_sha256_url}"
    fi

    if [ ! -f "/usr/src/cmake-${cmake_latest_ver}-linux-x86_64.tar.gz" ]; then
        retry curl -ksSLo "/usr/src/cmake-${cmake_latest_ver}-linux-x86_64.tar.gz" "${cmake_binary_url}"
        if retry curl -ksSL --compressed "${cmake_sha256_url}" \| grep "cmake-${cmake_latest_ver}-linux-x86_64.tar.gz" \| sha256sum -c; then
            ...
            rm -rf /usr/src/cmake-${cmake_latest_ver}-linux-x86_64.tar.gz
        fi
    fi
